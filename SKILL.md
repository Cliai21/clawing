...
license: MIT-0
metadata: {"author": "Cliai21", "version": "3.0", "network": "Ethereum Mainnet", "github": "https://github.com/Cliai21/clawing", "openclaw": {"emoji": "⛏️", "homepage": "https://minewithclaw.com", "requires": {"bins": ["node", "git"], "env": ["AI_API_KEY"]}, "primaryEnv": "AI_API_KEY"}}
---
...
- The private key NEVER leaves the user's local machine
- All transactions are signed locally by the miner process running on the user's own computer
- The Skill (agent) writes the private key into a local `.env` file with `chmod 600` permissions
- The private key is NEVER sent to the Oracle, AI API, or any external service
- The agent must NEVER log, echo, or display the private key after receiving it

The miner code loads the private key into an `ethers.Wallet` object at startup and removes it from the config object immediately — this is a security design choice from the v2.1 audit.
...
Follow these steps in order. Ask the user for credentials as needed — never guess or skip security-sensitive values.

### Step 1: Obtain Credentials from the User

Ask the user for these 3 items (they can provide them one at a time):
...
3. **Mining wallet private key** — a hot wallet with some ETH for gas (NOT their main wallet/Ledger)

**IMPORTANT**: The code uses the environment variable `PRIVATE_KEY` (not `MINER_PRIVATE_KEY`).
...
Create the `.env` file in the `miner/` directory. There are two methods:
...
This will prompt for each value interactively. When asked:
