...
**Your keys stay local.** The private key never leaves your machine. All transactions are signed locally by the miner process running on your own computer.
...
3. **Mining wallet private key** — a dedicated hot wallet with some ETH for gas
...
A: Yes. Your private key never leaves your local machine. All transactions are signed locally by the miner process. The key is never sent to the Oracle, AI API, or any external service.
